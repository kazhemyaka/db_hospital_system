<?php
    ...
    if (isset($_POST['submit_diagnosis'])) {
        $new_patient_id = $_POST['add_diagnosis_patient'];
        $new_name = $_POST['add_diagnosis_name'];
        $new_notes = $_POST['add_diagnosis_notes'];
        $new_file = $_POST['add_diagnosis_file'];
        $diag_update_query = "INSERT INTO Diagnosis (Doctor_ID, Patient_ID, Diagnosis_Name, Diagnosis_Notes, Diagnosis_File) VALUES ('{$_SESSION['doctor_id']}', '$new_patient_id', '$new_name', '$new_notes', '$new_file')";
...
<head>
    ...
    <title>Adding a new diagnosis - Dashboard</title>
...
<div class="d-flex flex-nowrap">
    ...
    <div class="container">
        <div class="px-4 py-5">
            <h2 class="mb-4">Adding a new diagnosis</h2>
            <div id="add_diagnosis">
                <form method="POST" action="new_diagnosis.php">
                    <label for="add_diagnosis_patient" class="form-label">Patient number:</label>
                    <input type="text" pattern="^[1-9]\d*$" name="add_diagnosis_patient" class="form-control" id="add_diagnosis_patient" required>
                    <label for="add_diagnosis_name" class="form-label">Diagnosis name:</label>
                    <input type="text" name="add_diagnosis_name" class="form-control" id="add_diagnosis_name" required>
                    <label for="add_diagnosis_notes" class="form-label mt-2">Diagnosis notes:</label>
                    <textarea type="text" name="add_diagnosis_notes" class="form-control" id="add_diagnosis_notes" required></textarea>
                    <label for="add_diagnosis_file" class="form-label mt-2">File:</label>
                    <input type="text" name="add_diagnosis_file" class="form-control" id="add_diagnosis_file" value="../diagnoses/" required>
                    <input type="submit" value="Save" name="submit_diagnosis" class="btn btn-primary mt-4">
                </form>
            </div>
        </div>
    </div>
